...
class pyMainwindow(QMainWindow):
    ...
    def initUI(self):
        self.ui_w = 320
        self.ui_h = 240
        self.show_img.resize(self.ui_w, self.ui_h)
    ...
    def Mat2QImage(self, label, img):
        img = cv2.resize(img, dsize=(self.ui_w, self.ui_h))
...
class NO_GUI:
    ...
    def main(self):
        while self.cap.isOpened():
            ...
            if len(boxes) != 0:
                new_bbox = utils.convert_bbox_resolution(boxes, self.model_input, self.cam_output)
                utils.draw(img, new_bbox, confs, classes, self.color_list, self.clss, self.msg_pub, self.obj, self.args.node)
            sec = curTime - self.prevTime
            self.prevTime = curTime
            if (key == '\x03'):
                break
...
def main():
    ...
    if opt.app:
        ...
        timer.start(33)
        pyShow.setWindowTitle("WINDOW  " + opt.node)
        pyShow.show()
        sys.exit(app.exec_())
    else:
        print("Not Use GUI")
